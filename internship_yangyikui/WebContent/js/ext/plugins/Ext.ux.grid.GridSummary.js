    ...
    renderSummary : function(o, cs, cm) {
        ...
        for (var i = 0, len = cs.length; i < len; i++) {
            ...
            if (cf.summaryType || cf.summaryRenderer) {
            	//modify by likun 2010-12-28 begin 如果是求和类型合计，则将该值保留两位小数，避免小数失真情况，并且将小数末尾的0去掉（通过paresFloat）
            	var summaryValue = o.data[c.name];
            	if(cf.summaryType == 'sum'){
            		summaryValue = parseFloat(summaryValue.toFixed(2));
            	}
                p.value = (cf.summaryRenderer || c.renderer)(summaryValue, p, o);
                //end 
            } else {
                p.value = '';
            }
